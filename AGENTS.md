...
| [`wallet-observability`](.claude/skills/wallet-observability/SKILL.md) | Editing `internal/metrics/` or `internal/tracing/`, adding a counter/histogram/gauge, wrapping a method in a span, anything Prometheus / OpenTelemetry / `/metrics` | Metric naming conventions, label-cardinality safety, span / attribute templates, exporter configuration, admin-listener gating |
| [`wallet-vuln-triage`](.claude/skills/wallet-vuln-triage/SKILL.md) | Triaging a Dependabot alert, CodeQL finding, govulncheck hit, or upstream CVE | Two-question test (reachable? exploitable?), `govulncheck -mode=source` workflow, dismiss-with-justification template, decision matrix |
| [`wallet-deps`](.claude/skills/wallet-deps/SKILL.md) | Changing `go.mod` / `go.sum`, bumping a dep, adding an import, interpreting a `go mod tidy` rewrite | Last-known-1.24-compatible version table for the key deps, escalation procedure for floor changes, anti-patterns around `replace` directives |
| [`wallet-ci-workflow`](.claude/skills/wallet-ci-workflow/SKILL.md) | Editing `.github/workflows/`, opening a PR, debugging CI, composing a commit message | What's wired in CI today, branch / PR / commit conventions, the pre-push `make security-check` gate, what to do when CI fails |
| [`wallet-perf`](.claude/skills/wallet-perf/SKILL.md) | Profiling, running benchmarks / k6 / stress tests, tuning the DB pool, diagnosing latency or throughput regressions | Decision table for benchmark vs k6 vs pprof vs stress, baseline numbers from the README, pool-tuning guidance, anti-patterns |
